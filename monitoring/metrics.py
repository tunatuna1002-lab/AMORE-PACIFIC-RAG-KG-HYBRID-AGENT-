...
class QualityMetrics:
    ...
    def stop_timer(self, name: str, labels: Optional[Dict] = None) -> Optional[float]:
        """타이머 종료 및 히스토그램에 기록"""
    ...
    def record_agent_complete(self, agent_name: str, result: Optional[Dict] = None) -> None:
        """에이전트 완료 기록"""
    ...
    def record_agent_error(self, agent_name: str, error: str) -> None:
        ...
        if agent_name in self._agent_metrics:
            ...
            self.histogram("agent_duration_seconds", duration, {"agent": agent_name, "status": "error"})
    ...
    def load(self, date: Optional[str] = None) -> bool:
        """메트릭 로드"""
        if date is None:
