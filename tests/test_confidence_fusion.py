...
def test_fuse_all_sources(sample_vector_results, sample_ontology_results, sample_entity_links):
    ...
    for source in result.source_scores:
        print(f"  {source.source_name}: raw={source.raw_score:.3f}, "
...
def test_softmax_normalization(sample_vector_results, sample_ontology_results):
    ...
    for source in result.source_scores:
        print(f"{source.source_name}: {source.normalized_score:.3f} "
              f"(sum={normalized_sum:.3f})")
...
def test_conflict_detection():
    ...
    # 매우 높은 벡터 점수
    vector_results = [
        SearchResult(content="High score document", score=0.95, metadata={})
    ]
...
def test_high_confidence_scenario():
    """높은 신뢰도 시나리오 (정규화 없음)"""
    vector_results = [
        SearchResult(content="Perfect match", score=0.95, metadata={})
    ]
    ontology_results = [
...
def test_low_confidence_scenario():
    """낮은 신뢰도 시나리오 (정규화 없음)"""
    vector_results = [
        SearchResult(content="Weak match", score=0.35, metadata={})
    ]
    ontology_results = [
