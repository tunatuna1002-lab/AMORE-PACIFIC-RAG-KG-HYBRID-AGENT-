...
async def test_send_pending_alerts_result_keys(alert_agent):
    ...
    # FIXED (D6): the result dict carries both "sent" and the compatibility alias
    # "sent_count" (read by AlertWorkflow); the two are always equal.
    assert set(result.keys()) == {"processed", "sent", "sent_count", "failed", "skipped", "details"}
    assert result["sent_count"] == result["sent"]
...
@pytest.mark.asyncio
async def test_metrics_agent_execute_finds_history_keyed_by_asin(tmp_path):
    # FIXED (D5): MetricsAgent.execute now falls back from "product_asin" to "asin", so a
    # crawl snapshot keyed by "asin" finds its history -> rank_change_1d=-14, a rank_shock
    # alert (and a top10_entry alert now that history exists).
    ...
    assert first["rank_change_1d"] == -14  # yesterday 15 -> today 1
    assert first["rank_change_7d"] is None  # only one history point
    assert first["rank_volatility"] == 7.0
    ...
    assert result["alerts"] == [
        {
            ...
            "message": "Top 10 진입 (1위)",
            "details": {"current_rank": 1, "category": "lip_care"},
        },
        {
            "type": "rank_shock",
            "severity": "warning",
            "asin": "B000000001",
            "title": "LANEIGE product 1",
            "message": "순위 급변동 감지 (변동폭: 14)",
            "details": {"is_shock": True, "change": -14, "previous_rank": 15, "current_rank": 1},
        },
    ]
