...
def test_query_intent_classification(results: TestResult):
    ...
    try:
        from src.rag.hybrid_retriever import (
            QueryIntent, classify_intent, get_doc_type_filter,
        ...
        for query in diagnosis_queries:
            ...
            assert intent == QueryIntent.DIAGNOSIS, f"Expected DIAGNOSIS for '{query}', got {intent}"
...
def test_document_metadata(results: TestResult):
    ...
    try:
        ...
        # 필수 메타데이터 필드 확인
        required_fields = ["filename", "description", "keywords", "doc_type", "intent_triggers", "freshness"]
        ...
        assert type_counts["metric_guide"] == 4, f"Expected 4 metric_guide, got {type_counts['metric_guide']}"
        assert type_counts["playbook"] == 2, f"Expected 2 playbook, got {type_counts['playbook']}"
        assert type_counts["intelligence"] == 2, f"Expected 2 intelligence, got {type_counts['intelligence']}"
        assert type_counts["knowledge_base"] == 1, f"Expected 1 knowledge_base, got {type_counts['knowledge_base']}"
        assert type_counts["response_guide"] == 2, f"Expected 2 response_guide, got {type_counts['response_guide']}"
        results.record_pass(f"문서 유형별 개수: metric_guide={type_counts['metric_guide']}, playbook={type_counts['playbook']}, intelligence={type_counts['intelligence']}, knowledge_base={type_counts['knowledge_base']}, response_guide={type_counts['response_guide']}")
...
async def test_intent_based_search(results: TestResult, retriever):
    ...
    try:
        from src.rag.hybrid_retriever import classify_intent, get_doc_type_filter, QueryIntent
        ...
        if search_results:
            ...
            assert first_doc_type in ["playbook", "metric_guide", "intelligence"], \
        ...
        if search_results:
            first_doc_type = search_results[0]["metadata"].get("doc_type")
            assert first_doc_type in ["intelligence", "knowledge_base", "response_guide"], \
        ...
        if search_results:
            first_doc_type = search_results[0]["metadata"].get("doc_type")
            assert first_doc_type in ["response_guide", "intelligence", "playbook"], \
...
async def test_hybrid_retriever_integration(results: TestResult):
    ...
    try:
        from src.rag.hybrid_retriever import HybridRetriever, classify_intent, QueryIntent
        ...
        context = await hybrid_retriever.retrieve(
            query,
            current_metrics={"sos": 0.15, "hhi": 0.22}
        )
