...
class InsightVerifier:
    ...
    async def _verify_rank_comparisons(
        ...
        rank_pattern = r'(\d+)\s*위에서\s*(\d+)\s*위로|(\d+)위\s*→\s*(\d+)위|순위:\s*(\d+)\s*→\s*(\d+)'
    ...
    def _verify_brand_names(
        ...
        # Unknown 브랜드 경고
        unknown_count = content.lower().count("unknown")
        if unknown_count > 3:
            issues.append(VerificationIssue(
                severity="warning",
                category="brand_name",
                description=f"'Unknown' 브랜드가 {unknown_count}회 언급되었습니다.",
                location="전체",
                original_text="Unknown",
                suggested_fix="Unknown 브랜드는 '기타/미확인 브랜드'로 표현하거나 분석에서 제외 고려"
            ))
