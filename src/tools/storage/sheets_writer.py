...
class SheetsWriter:
    ...
    def __init__(self, credentials_path: str | None = None, spreadsheet_id: str | None = None):
        ...
        # 제품 목록 캐시 (API 호출 최소화)
        self._products_cache: dict[str, dict[str, Any]] | None = None
    ...
    def _get_credentials(self) -> Credentials:
        ...
        if self.credentials_json:
            ...
            logger.info(
                "Loading credentials from GOOGLE_SHEETS_CREDENTIALS_JSON environment variable"
            )
            try:
                ...
                raise ValueError(f"Invalid JSON in GOOGLE_SHEETS_CREDENTIALS_JSON: {e}") from e
        else:
            # 파일에서 로드 (로컬 개발용)
        ...
        try:
            ...
            for sheet_config in self.SHEETS_CONFIG.values():
                if sheet_config["name"] not in existing_sheets:
                    requests.append({"addSheet": {"properties": {"title": sheet_config["name"]}}})
            ...
            if requests:
                ...
                # 헤더 추가
                for _sheet_key, sheet_config in self.SHEETS_CONFIG.items():
                    if sheet_config["name"] not in existing_sheets:
    ...
    async def append_rank_records(self, records: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Appending {len(rows)} rows to {range_name} in spreadsheet {self.spreadsheet_id[:20]}..."
            )
            result = (
            ...
            return {"success": False, "error": str(e), "sheet": sheet_config["name"]}
        except Exception as e:
    ...
    async def get_rank_history(
        ...
        try:
            ...
            for row in values[1:]:
                record = dict(zip(headers, row + [""] * (len(headers) - len(row)), strict=False))
    ...
    async def get_product_info(self, asin: str) -> dict[str, Any] | None:
        """제품 정보 조회 (캐시 활용)"""
