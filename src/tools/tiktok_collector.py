...
class TikTokCollector:
    ...
    KBEAUTY_HASHTAGS = [
        ...
        "skincare", "skincareroutine", "glowyskin",
        "cosrx", "tirtir", "biodance", "anua"
    ]

    # 뷰티 인플루언서 탐지 키워드
    BEAUTY_KEYWORDS = [
        "skincare", "beauty", "makeup", "routine", "review",
        "glow", "hydration", "moisturizer", "serum", "cream"
    ]
    ...
    def generate_report_section(self) -> str:
        ...
        avg_engagement = sum(p.get_engagement_rate() for p in self._posts) / len(self._posts) if self._posts else 0
