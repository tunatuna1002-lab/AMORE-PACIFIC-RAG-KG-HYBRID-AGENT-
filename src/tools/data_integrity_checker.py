...
사용법:
    from src.tools.data_integrity_checker import DataIntegrityChecker
...
DEFAULT_SPREADSHEET_ID = os.environ.get(
    ...
    "1cNr3E2WSSbO83XXh_9V92jwc6nfsxjAogcswlHcjV9w"
)
...
class DataIntegrityChecker:
    ...
    def __init__(
        ...
        sqlite_path: str = "./data/amore_data.db"
    ):
        """
        Args:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if not self._initialized:
                logger.warning(
                    f"Initialization incomplete: Sheets={sheets_ok}, SQLite={sqlite_ok}"
                )
    ...
    async def check_sync_status(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            if sheets_records:
                sheets_dates = {
                    r.get("snapshot_date", "") for r in sheets_records
    ...
    async def get_missing_dates(self, days: int = 30) -> List[str]:
        ...
        try:
            ...
            sqlite_raw = await self._sqlite.get_raw_data(
            ...
            if sqlite_raw:
                sqlite_dates = {
                    r.get("snapshot_date", "") for r in sqlite_raw
                    if r.get("snapshot_date")
                }
    ...
    def _generate_recommendations(
        ...
        if gap > 100:
            recommendations.append(
                f"SQLite에 {gap}개 레코드 누락. "
                "`python scripts/sync_sheets_to_sqlite.py` 실행 권장"
            )
        ...
        if not sync_status.get("is_synced", True):
            recommendations.append(
                "Google Sheets → SQLite 자동 동기화 설정 확인 필요"
            )
