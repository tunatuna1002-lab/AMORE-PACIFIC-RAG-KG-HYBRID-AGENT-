...
class GoogleTrendsCollector:
    ...
    COMPETITOR_KEYWORDS = ["LANEIGE", "COSRX", "Beauty of Joseon", "Innisfree", "TIRTIR"]
    ...
    def __init__(self, geo: str = "US", timeframe: str = "today 3-m"):
        ...
        self._trendspyg: TrendsPyG | None = None
    ...
    async def fetch_trends(self, keywords: list[str], geo: str | None = None) -> list[TrendData]:
        """
        키워드 리스트의 트렌드 데이터 수집
        ...
        try:
            ...
            logger.info(
                f"Fetched trends for {len(keywords)} keywords (geo={geo}, backend={self._backend})"
            )
            ...
            for keyword in keywords:
                trends.append(
                    TrendData(keyword=keyword, geo=geo, timeframe=self.timeframe, collected_at=now)
                )

        return trends

    async def _fetch_with_trendspyg(
        self, keywords: list[str], geo: str, now: str
    ) -> list[TrendData]:
            ...
            for keyword in keywords:
                try:
                    ...
                    data = trendspyg.interest_over_time(
                        keyword=keyword, geo=geo if geo else "US", timeframe=self.timeframe
                    )
                    result[keyword] = data
                except Exception as e:
                    ...
                    interest_data.append(
                        {
                            ...
                            "value": int(
                                row.get(keyword, row.iloc[0]) if hasattr(row, "get") else row
                            ),
                        }
                    )
    ...
    async def _fetch_with_pytrends(
        ...
        def _fetch():
            pytrends.build_payload(kw_list=keywords, cat=0, timeframe=self.timeframe, geo=geo)
        ...
        for keyword in keywords:
            ...
            if not interest_df.empty and keyword in interest_df.columns:
                for date, row in interest_df.iterrows():
                    interest_data.append({"date": str(date.date()), "value": int(row[keyword])})
        ...
        for trend in trends:
            ...
            if len(recent_values) >= 2:
                ...
                if prev > 0:
                    ...
                    lines.append(
                        f"- **{trend.keyword}**: {icon} {change_str} (최근 관심도: {latest})"
                    )
    ...
    async def save_trends(self, trends: list[TrendData], filename: str | None = None) -> Path:
        """
        트렌드 데이터 저장
    ...
    def load_latest_trends(self) -> dict[str, Any] | None:
        """가장 최근 트렌드 데이터 로드"""
