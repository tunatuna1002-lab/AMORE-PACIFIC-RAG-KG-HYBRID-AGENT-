...
class PublicDataCollector:
    ...
    def __init__(self, api_key: str | None = None, data_dir: str = "./data/public_data"):
        """
        Args:
    ...
    async def fetch_trade_by_country(
        self, country_code: str, year: str, month: str | None = None, trade_type: str = "export"
    ) -> TradeData | None:
        """
        특정 국가 대상 수출입 통계 조회
        ...
        try:
            ...
            trade = TradeData(
                ...
                unit=item.get("unitCd"),
                source_url=endpoint,
                metadata={"raw_response": item, "country_info": country_info},
            )
    ...
    async def fetch_us_cosmetics_export(
        self, year: str, month: str | None = None
    ) -> TradeData | None:
        """
        ...
        return await self.fetch_trade_by_country(
            country_code="US", year=year, month=month, trade_type="export"
        )
    ...
    async def fetch_functional_cosmetics(
        ...
        try:
            logger.info(
                f"Fetching functional cosmetics: company={company_name}, product={product_name}"
            )
    ...
    def get_trade_summary(self, year: str, trade_type: str = "export") -> dict[str, Any]:
        """
        수출입 통계 요약
        ...
        Returns:
            요약 딕셔너리
        """
        filtered = [t for t in self.trade_data if t.year == year and t.trade_type == trade_type]
        ...
        # 상위 5개국
        top_countries = sorted(by_country.items(), key=lambda x: x[1]["amount_usd"], reverse=True)[
            :5
        ]
    ...
    def generate_insight_section(self) -> str:
        ...
        # 최근 수출입 데이터
        recent_trades = sorted(self.trade_data, key=lambda x: (x.year, x.month), reverse=True)[:5]
        ...
        if self.cosmetics_products:
            recent_products = len(
                [
                    p
                    for p in self.cosmetics_products
    ...
    def create_source_reference(
        self, trade_data: TradeData | None = None, product_data: CosmeticsProduct | None = None
    ) -> dict[str, Any]:
        """
        ...
        if trade_data:
            period = (
                f"{trade_data.year}.{trade_data.month}"
                if trade_data.month != "00"
                else trade_data.year
            )
            return {
    ...
    def _load_data(self) -> None:
        ...
        if trade_file.exists():
            try:
                with open(trade_file, encoding="utf-8") as f:
                    data = json.load(f)
                self.trade_data = [TradeData(**t) for t in data.get("trade_records", [])]
        ...
        if cosmetics_file.exists():
            try:
                with open(cosmetics_file, encoding="utf-8") as f:
                    data = json.load(f)
                self.cosmetics_products = [CosmeticsProduct(**p) for p in data.get("products", [])]
