...
class AlertService:
    ...
    def __init__(self, config: AlertConfig | None = None):
        ...
        self._email_enabled = bool(
            self.config.smtp_host and self.config.smtp_user and self.config.email_recipients
        )
    ...
    async def process_deals_for_alerts(self, deals: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """
    ...
    def _check_deal_for_alert(self, deal: dict[str, Any]) -> dict[str, Any] | None:
        """개별 딜에 대한 알림 조건 체크"""
    ...
    async def _send_slack_batch(self, alerts: list[dict[str, Any]]) -> bool:
        ...
        for alert in alerts[:10]:
            ...
            product_name = (
                (alert.get("product_name", "")[:50] + "...")
                if len(alert.get("product_name", "")) > 50
                else alert.get("product_name", "")
            )
            ...
            blocks.append(
                {
                    ...
                    "accessory": {
                        ...
                        or f"https://amazon.com/dp/{alert.get('asin')}",
                    },
                }
            )
    ...
    async def _send_email_batch(self, alerts: list[dict[str, Any]]) -> bool:
        ...
        msg["Subject"] = (
            f"[AMORE] 경쟁사 할인 알림 - {len(alerts)}건 ({datetime.now(KST).strftime('%m/%d')})"
        )
        ...
        try:
            with smtplib.SMTP(self.config.smtp_host, self.config.smtp_port) as server:
                ...
                server.sendmail(
                    self.config.email_from, self.config.email_recipients, msg.as_string()
                )
            logger.info(
                f"Email alert sent: {len(alerts)} deals to {len(self.config.email_recipients)} recipients"
            )
            return True
        except Exception as e:
    ...
    def _build_email_html(self, alerts: list[dict[str, Any]]) -> str:
        ...
        for alert in alerts[:20]:
            ...
            discount = alert.get("discount_percent", 0)
            product_name = (
                (alert.get("product_name", "")[:40] + "...")
                if len(alert.get("product_name", "")) > 40
                else alert.get("product_name", "")
            )
