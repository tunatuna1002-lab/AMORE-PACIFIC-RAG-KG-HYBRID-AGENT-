...
class QuarterlyFinancials:
    ...
    net_income_krw: float | None = None
...
class IRReportParser:
    ...
    async def download_pdf(self, url: str, filename: str | None = None) -> Path | None:
        """
        PDF 파일 다운로드
    ...
    def extract_financials_from_text(
        self, text: str, year: str, quarter: str
    ) -> QuarterlyFinancials | None:
        """
        텍스트에서 재무 데이터 추출
    ...
    def get_quarterly_data(self, year: str, quarter: str) -> IRReport | None:
        """
        특정 분기 데이터 조회
    ...
    def get_latest_report(self) -> IRReport | None:
        """
        가장 최근 보고서 조회
        ...
        sorted_reports = sorted(
            self.reports.values(), key=lambda r: (r.year, r.quarter), reverse=True
        )

        return sorted_reports[0] if sorted_reports else None

    def get_regional_performance(
        self, region: str, year: str, quarter: str
    ...
    def get_brand_highlights(
        self, brand: str, year: str | None = None, quarter: str | None = None
    ) -> list[BrandHighlight]:
        """
        브랜드별 하이라이트 조회
    ...
    def get_americas_insights(
        ...
        Returns:
            Americas 인사이트 딕셔너리
        """
        insights = {"regional_performance": [], "brand_highlights": [], "key_events": []}
                ...
                for event in key_events:
                    if "amazon" in event.lower() or "prime" in event.lower():
                        insights["key_events"].append(
                            {"year": report.year, "quarter": report.quarter, "event": event}
                        )
    ...
    def generate_insight_section(self, year: str | None = None, quarter: str | None = None) -> str:
        """
        인사이트 보고서용 섹션 생성
        ...
        if report.financials:
            ...
            revenue_yoy = (
                f"+{f.revenue_yoy:.1f}%"
                if f.revenue_yoy and f.revenue_yoy >= 0
                else f"{f.revenue_yoy:.1f}%"
            )
        ...
        if americas:
            yoy_str = (
                f"+{americas.revenue_yoy:.1f}%"
                if americas.revenue_yoy >= 0
                else f"{americas.revenue_yoy:.1f}%"
            )
    ...
    def create_source_reference(self, year: str, quarter: str) -> dict[str, Any]:
        """
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "years_covered": list({r.year for r in self.reports.values()}),
            "latest_report": self.get_latest_report().report_id
