...
from .ir_report_parser import IRReportParser
...
class MarketIntelligenceEngine:
    ...
    def __init__(
        self, public_data_api_key: str | None = None, data_dir: str = "./data/market_intelligence"
    ):
        """
        Args:
        ...
        self.public_collector = PublicDataCollector(
            api_key=public_data_api_key, data_dir=str(self.data_dir / "public_data")
        )
        self.ir_parser = IRReportParser(data_dir=str(self.data_dir / "ir_reports"))
        self.signal_collector = ExternalSignalCollector(data_dir=str(self.data_dir / "signals"))

        # 출처 관리자
        self.source_manager = SourceManager(data_dir=str(self.data_dir / "sources"))
    ...
    async def collect_layer_4_macro(
        self, year: str | None = None, month: str | None = None
    ) -> LayerData:
        """
        ...
        data = {"us_export": None, "total_export": [], "trade_summary": None}
        sources = []
        ...
        layer_data = LayerData(
            layer=DataLayer.LAYER_4_MACRO, layer_name="거시경제/무역", data=data, sources=sources
        )
    ...
    async def collect_layer_3_industry(
        self, year: str | None = None, quarter: str | None = None
    ) -> LayerData:
        """
        ...
        Returns:
            LayerData
        """
        data = {"ir_report": None, "americas_insights": None, "brand_highlights": {}}
        sources = []
        ...
        try:
            ...
            if report:
                ...
                # LANEIGE 하이라이트
                laneige = self.ir_parser.get_brand_highlights(
                    "LANEIGE", report.year, report.quarter
                )
                if laneige:
        ...
        layer_data = LayerData(
            layer=DataLayer.LAYER_3_INDUSTRY, layer_name="산업/기업", data=data, sources=sources
        )

        self.layer_data[DataLayer.LAYER_3_INDUSTRY] = layer_data
        return layer_data

    async def collect_layer_2_consumer(self, keywords: list[str] | None = None) -> LayerData:
        """
        ...
        Returns:
            LayerData
        """
        data = {"kbeauty_news": [], "reddit_trends": [], "industry_signals": []}
        sources = []
        ...
        layer_data = LayerData(
            layer=DataLayer.LAYER_2_CONSUMER, layer_name="소비자 트렌드", data=data, sources=sources
        )

        self.layer_data[DataLayer.LAYER_2_CONSUMER] = layer_data
        return layer_data

    async def collect_layer(self, layer_number: int, **kwargs) -> LayerData | None:
        """
        특정 레이어 데이터 수집
    ...
    def generate_layered_insight(self, amazon_data: dict[str, Any] | None = None) -> str:
        """
        4-Layer 기반 인사이트 생성
        ...
        sections.append("# LANEIGE Amazon US 일일 인사이트")
            ...
            if us_export:
                ...
                if amount:
                    ...
                    source = self.source_manager.add_source(
                        title="품목별 수출입통계 (HS Code 3304)",
                        publisher="관세청",
                        date=f"{us_export.get('year')}.{us_export.get('month')}",
                        source_type="government",
                    )
                    sections.append(
                        f"• 화장품 대미 수출: {amount_str}{yoy_str} {source.to_citation()}"
                    )
        ...
        if layer3 and layer3.data:
            ...
            if ir_report:
                ...
                if regional_perf:
                    ...
                    sections.append(
                        f"• 아모레퍼시픽 Americas: {revenue:.1f}B KRW ({yoy_str} YoY) {source.to_citation()}"
                    )
        ...
        if layer2 and layer2.data:
            ...
            if reddit:
                ...
                if total_upvotes > 0:
                    sections.append(
                        f"• Reddit: 최근 K-Beauty 관련 게시물 {len(reddit)}건 (누적 {total_upvotes:,} 업보트)"
                    )
...
# 편의 함수
async def create_market_intelligence_engine(api_key: str | None = None) -> MarketIntelligenceEngine:
    """
