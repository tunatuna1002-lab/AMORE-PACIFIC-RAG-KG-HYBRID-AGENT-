...
class BrandResolver:
    ...
    def get_brand(self, asin: str) -> Optional[str]:
        """
        매핑 테이블에서 브랜드 조회 (O(1))
    ...
    def add_mapping(self, asin: str, brand: str, product_name: str = "", auto_save: bool = True) -> None:
        """
        브랜드 매핑 추가
    ...
    async def _fetch_brand_from_amazon(self, asin: str) -> Optional[str]:
        """
    ...
    async def _search_brand_web(self, product_name: str) -> Optional[str]:
        """
        웹검색으로 브랜드 확인
...
JSON 형식으로만 응답: {{"brand": "브랜드명"}}"""
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_brands": sorted(
                [(b, brands.count(b)) for b in unique_brands],
                key=lambda x: x[1],
                reverse=True
            )[:10]
        }
...
# 싱글톤 인스턴스
_resolver_instance: Optional[BrandResolver] = None
