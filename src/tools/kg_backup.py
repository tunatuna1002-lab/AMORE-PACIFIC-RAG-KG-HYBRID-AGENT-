...
## 사용 예
```python
from src.tools.kg_backup import KGBackupService
...
class KGBackupService:
    ...
    def __init__(
        ...
        if backup_dir:
            ...
            logger.info(
                "Railway environment detected, using Volume backup path: /data/backups/kg"
            )
        else:
    ...
    def list_backups(self) -> List[Dict[str, Any]]:
        ...
        for backup_file in sorted(
            self.backup_dir.glob("kg_backup_*.json"), reverse=True
        ):
            if ".meta" in str(backup_file):
                continue
...
_backup_service_instance: Optional[KGBackupService] = None
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "command", choices=["backup", "restore", "list", "cleanup", "stats"]
    )
