...
class YouTubeCollector:
    ...
    def _parse_entry(self, entry: Dict[str, Any]) -> Optional[YouTubeVideo]:
    ...
    async def search(self, query: str, limit: int = 20) -> List[YouTubeVideo]:
        ...
        videos = await loop.run_in_executor(
            ...
            self._search_sync,
            query,
            min(limit, self.max_results_per_query)
        )
    ...
    def generate_report_section(self) -> str:
        ...
        avg_duration = sum(v.duration for v in self._videos) / len(self._videos) if self._videos else 0
