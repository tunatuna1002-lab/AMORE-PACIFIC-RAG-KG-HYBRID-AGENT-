...
class ConfidenceFusion:
    ...
    def fuse(
        ...
        # 5. 최종 신뢰도 계산
        final_confidence = self._apply_fusion_strategy(normalized_scores, adjusted_weights)
        ...
        # 8. 문서 통합
        documents = self._merge_documents(vector_results, ontology_results, entity_links)

        # 9. 설명 생성
        explanation = self._generate_explanation(source_scores, final_confidence, query)
    ...
    def _normalize_scores(self, scores: dict[str, float]) -> dict[str, float]:
        ...
        if self.normalization == ScoreNormalizationMethod.MIN_MAX:
            ...
            return dict(zip(scores.keys(), softmax, strict=False))
    ...
    def _adjust_weights(self, available_sources: dict[str, Any]) -> dict[str, float]:
        ...
        """
        active_weights = {k: v for k, v in self.weights.items() if k in available_sources}
    ...
    def _apply_fusion_strategy(self, scores: dict[str, float], weights: dict[str, float]) -> float:
        """융합 전략 적용"""
        if not scores:
            return 0.0
        ...
        if self.strategy == FusionStrategy.WEIGHTED_SUM:
            # 가중합
            return sum(s * w for s, w in zip(values, weight_list, strict=False))

        elif self.strategy == FusionStrategy.HARMONIC_MEAN:
            # 조화평균 (모든 점수가 높아야 높음 - 보수적)
            if any(v == 0 for v in values):
                return 0.0
            weighted_reciprocals = sum(w / v for v, w in zip(values, weight_list, strict=False))
            return 1.0 / weighted_reciprocals if weighted_reciprocals > 0 else 0.0

        elif self.strategy == FusionStrategy.GEOMETRIC_MEAN:
            # 기하평균
            product = np.prod([v**w for v, w in zip(values, weight_list, strict=False)])
            return product
            ...
            rrf_score = sum(1.0 / (k + rank + 1) for rank, _ in enumerate(sorted_vals))
            return min(rrf_score, 1.0)
    ...
    def _generate_explanation(
        self, source_scores: list[SourceScore], final_confidence: float, query: str | None
    ) -> str:
        """종합 설명 생성"""
        overall_level = self._get_confidence_level(final_confidence)

        # 기여도 높은 순으로 정렬
        sorted_sources = sorted(source_scores, key=lambda s: s.contribution, reverse=True)
        ...
        if major_contributors:
            contrib_strs = [f"{s.source_name}({s.contribution:.2f})" for s in major_contributors]
            explanation_parts.append(f"주요 근거: {', '.join(contrib_strs)}")
