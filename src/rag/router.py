...
class RAGRouter:
    ...
    QUERY_PATTERNS = {
        ...
        QueryType.DATA_QUERY: {
            ...
            # IR 분기 실적 어휘 (2026-08-30 사이클 7).
            # 코퍼스에 IR 분기보고서가 있고 검색도 되는데, 라우터가 이 어휘를
            # 하나도 몰라 "질문의 의도를 정확히 파악하지 못했습니다" 폴백으로
            # **검색 전에 거절**하고 있었다 (IR 골든셋 12문항 중 9문항).
            "patterns": [
                "매출",
                "영업이익",
                "순이익",
    ...
    def classify_query_detailed(self, query: str) -> tuple[QueryType, float, float, list[str]]:
        ...
        scores = dict.fromkeys(QueryType, 0.0)
        matched_keywords = []
    ...
    def get_target_document(self, query_type: QueryType) -> str | None:
        """
        질의 유형에 따른 대상 문서 반환
