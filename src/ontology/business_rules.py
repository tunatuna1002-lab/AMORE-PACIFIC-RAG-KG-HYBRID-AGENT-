...
# =========================================================================
# IR 크로스 분석 규칙 (IR-Amazon Cross Analysis Rules) - 2026-01-26 추가
# =========================================================================
...
RULE_IR_GROWTH_SLOWDOWN = InferenceRule(
    ...
    conditions=[
        RuleCondition(
            ...
            ) if ctx.get("ir_prev_qtr_growth", 0) > 0 else False,
    ...
    conclusion=lambda ctx: {
        ...
        "metadata": {
            ...
            "decline_ratio": (ctx.get("ir_prev_qtr_growth", 1) - ctx.get("ir_current_qtr_growth", 0)) / ctx.get("ir_prev_qtr_growth", 1) * 100 if ctx.get("ir_prev_qtr_growth", 0) > 0 else 0
        }
    },
    insight_type=InsightType.RISK_ALERT,
    priority=8,
...
# ALL_BUSINESS_RULES에 IR 규칙 추가
ALL_BUSINESS_RULES.extend(IR_CROSS_ANALYSIS_RULES)
