...
| **[2026-09-18 사후]** 서비스에 한 번도 연결되지 않았던 OWL 모듈(소스·OWL 2,644줄)을 삭제하고, 온톨로지 원본을 JSON + Python 폐포 로더로 다시 만들어 Pellet으로 교차 검증했다(불일치 0). 질의 경로에서 온톨로지(클래스·그룹·세그먼트·원산지) 기반 질의 확장·정적 사실 카드를 플래그 뒤에 붙이고, 54문항 × 3회로 OFF/ON을 비교해 기준을 넘은 개선을 확인한 뒤 기본 ON으로 바꿨다: 종합 0.692→0.745, L3 골드 엣지 recall(canonical) 0.301→0.582, 근거성 0.867→0.968, rule 42문항 규칙 정답 일치율 0.781→0.906. 대가(프롬프트 카드 58.1→69.9장, 비용 +9%)와 한계(통과 수는 노이즈 기준 미달, KG 저장 형식 마이그레이션은 효과 없음, 1문항 일관 하락)도 함께 기록 | `docs/experiments/ontology_activation_2026-09.md` §O7·§요약, 결정 OA-10~OA-13 — **공모전 이후 작업임을 명시** |
...
| "OWL 온톨로지 추론으로 검색한다" | OWL 전략 [미연결]. 실제 추론은 Python 규칙 엔진. **[2026-09 사후]** 연결 후 측정에서 필터 결함으로 124/130문항 문서 0건, 기본 OFF. **[2026-09-18 사후]** 전략 자체를 삭제했다 — 이제 OWL은 카테고리 계층 어휘로만 쓰이고 검색에 관여하지 않는다. **[2026-09 사후 정정, O6]** "카테고리 계층 어휘"도 사실과 달랐다 — OWL 모듈은 삭제됐고 카테고리 계층은 `config/category_hierarchy.json` | "규칙 기반 추론기(37개 규칙, 증거 카드 입력)를 검색 컨텍스트에 연결했다. 온톨로지 원본은 JSON + Python 폐포 로더이고, OWL은 개발용 Pellet 교차 검증에만 쓴다. 질의 경로의 온톨로지 클래스 추론은 플래그 뒤(기본 OFF, 효과 미측정)" (**[2026-09-18 사후, O7]** 뒷부분을 이렇게 바꿀 것: "질의 경로에서는 온톨로지(클래스·그룹·세그먼트·원산지) 기반 질의 확장·정적 사실 카드로 쓰고, 54문항 3회 측정에서 종합 0.692→0.745·L3 골드 엣지 recall 0.301→0.582를 확인해 기본 ON으로 바꿨다. 통과 문항 수 증가는 노이즈 기준을 넘지 못했다." "OWL 추론"이라는 말은 쓰지 않는다) ([2026-09 사후] 옛 권장 문구 "OWL 스키마는 카테고리 계층 검증에 쓴다"는 폐기) — 2026-09-17 측정 시점(규칙 추론 0건)과 2026-09-18 재작업 이후(발화 0.568, 정답 일치율 0.779, 단 judge 종합 점수는 규칙 off가 더 높음)를 구분해서 쓸 것(§5.5) |
