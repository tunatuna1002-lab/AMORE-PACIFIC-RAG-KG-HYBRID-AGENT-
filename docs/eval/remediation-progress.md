...
| 10-2 | 결측 지표로 규칙이 발동하지 않게 ("HHI: 0.000" 날조 제거) | done | 96673d1 | SoS·HHI·CPI 조건 6개를 기존 `_present` 규약으로, 인라인 조건 3곳, 추론 컨텍스트 기본값 주입 제거 |
| 10-3 | SQLite 지표를 질의 엔티티에 맞춰 컨텍스트에 싣기 (평가는 as_of 고정) | done | 3fce8e8 | src/rag/metric_facts.py. 무비용 점검: lg049·lg048·lg195·lg076 골드 수치가 컨텍스트에 들어감, lg072는 상위 5개 상한으로 불가. judge 근거성 컨텍스트에 DB 사실 포함 → groundedness v9.1과 비교 불가 |
| 10-4 | 1회 측정, v9.1과 비교 (**API 비용, 승인 필요**) | blocked | — | 승인 대기 — 사용자가 이번에는 실행하지 않기로 했다. 실행 명령·비교 가능 여부 표·예상 비용($0.42/회)은 사이클 10 문서 §5. 근거성은 judge 컨텍스트 정의가 바뀌어 재채점(~$0.11) 없이는 v9.1과 비교 불가 |
