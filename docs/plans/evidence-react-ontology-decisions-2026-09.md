...
| S2-2 | 2 | 테스트가 만든 KG 트리플 정리 (S0-7 후속) | **사용자 승인(2026-09-18)으로 삭제**: `LANEIGE hasProduct <ASIN>` 중 속성이 `product_name·rank·category`뿐인 6개(B0DHWFT182·B07XXPHQZK·B07DY2QRF6·B0DT2JNXP2·B0G7NRYFJS·B0GXGQL59K, source `system`, 생성 08-30·09-06). `KnowledgeGraph(persist_path, auto_save=False)` → `remove_relation` 6회 → `save(force=True)`. 3,500→3,494, 그 밖 추가·변경 0 확인. 백업 `eval_output/evidence-2026-09/kg_cleanup/knowledge_graph_before_delete6_2026-09-18.json`. 동결 평가 스냅샷(KG 876ea40f)은 기준선 비교를 위해 **바꾸지 않음**. 같은 테스트 패턴의 `LANEIGE competesWith <브랜드>`(속성 `competitor_sos`만, 08-30, source `system`) 14개(Aquaphor·Burt's Bees·CeraVe·Fresh·Hera·L'Oreal·La Roche-Posay·MEDICUBE·Maybelline·NYX·Revlon·Summer Fridays·e.l.f.·eos)도 사용자 추가 승인(2026-09-18)으로 같은 방법 삭제: 3,494→3,480, 그 밖 추가·변경 0. 백업 `kg_cleanup/knowledge_graph_before_delete14_2026-09-18.json` | `tests/test_llm_integration.py:build_knowledge_graph_from_dashboard`가 대문자 `LANEIGE` 주어로 hasProduct·belongsToCategory·competesWith를 추가. 같은 (s,p,o)의 `kg_enricher` belongsToCategory 트리플에는 테스트의 rank가 병합됐을 수 있으나 분리 불가라 유지 |
