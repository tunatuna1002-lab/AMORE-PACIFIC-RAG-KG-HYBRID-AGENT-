...
- 변경 2건 (2026-09-17 17:00경 KST, 사용자): 유료 API 총 상한 **$10 → $20** (경보선 $16).
  - 경위: 실험 중 `eval/cost_tracker.py`의 gpt-4.1-mini 단가가 실제의 약 1/2.67($0.15/$0.60 per 1M, 공시가 $0.40/$1.60)로 적혀 있어 리포트 비용이 과소 집계됨을 발견. 실제 단가로 재계산하니 ReAct·OWL 1회를 더하면 $10 상한의 경보선($8)을 넘어, D4 축소 순서 ①대로 ReAct·OWL 구성을 한때 생략했다. 상한 상향 후 확정안대로 1회 실행으로 되돌렸다.
- D1 최종값 (2026-09-17, 실험 후 판정): **`agents.use_react_agent`·`retriever.use_owl_strategy` 모두 OFF 유지** — "연결됨, 기본 비활성".
  - 근거: ReAct·OWL 1회(130문항)에서 ReAct 발동 0건(모든 문항이 신뢰도 HIGH라 ReAct 분기에 도달하지 않음)이라 ON 조건 "ReAct가 1문항 이상 발동"을 충족하지 못했다. 같은 실행에서 종합 −0.187·근거성 −0.313·통과 수 −10으로 노이즈 기준도 넘게 떨어졌는데, 원인은 ReAct가 아니라 OWL 검색 필터 결함(124문항 문서 0건)과 OWL 경로가 DB 지표·BM25·KG 사실 조회를 쓰지 않는 점이다. 1회 측정이고 두 변경이 섞여 있다. 상세: `docs/experiments/kg_ablation_2026-09.md` §6.
