...
**OE2-추가 (2026-09-18, JDK 25 설치 후 확정). 추론기는 Pellet 하나만 쓴다.**
- 로컬 기본 `java`는 Temurin 25.0.4.1이다. owlready2 0.50에 든 Pellet은 JDK 25 이상에서 동작한다(검토 보고서 §2.1-추가, 임시 예제 0.4초).
- Pellet은 클래스 분류·일관성 검사·SWRL 숫자 비교를 모두 하므로 HermiT는 쓰지 않는다. HermiT는 SWRL 내장 연산을 지원하지 않아, 기존 `.owl`의 규칙 3개도 돌리지 못한다.
- O1에 `scripts/check_ontology_owl.py`를 추가한다. 원본 스키마·등록부를 OWL로 내보낸 뒤 Pellet으로 두 가지를 확인한다.
  1. 일관성: 모순 클래스가 0개인지
  2. 교차 검증: 클래스 분류 결과가 Python 폐포와 같은지. 이 결과가 "Python 폐포가 OWL 의미론과 맞다"는 근거가 된다.
- 테스트는 Java 25 이상이 없으면 `skip`한다. 조용히 통과시키지 않고 건너뛴 사실을 출력에 남긴다(검토 보고서 §2.3 결함 3 재발 방지).
- OE1에서 OWL 원본을 택하면, 기존 `cosmetics_ontology.owl`의 파싱 오류(81행 `<`)부터 고쳐야 로드된다.

**OE10. 배포에서의 온톨로지 (2026-09-18 추가).**
- **서비스 질의 경로**는 JSON 원본 + Python 폐포로 동작한다. Java가 필요 없으므로 **배포 이미지를 바꾸지 않아도 O1~O4 기능이 배포에서 그대로 작동한다.**
  - 원본 파일(`config/ontology/*`)은 코드와 함께 이미지에 들어간다. `config/`는 볼륨이 아니다.
  - `scripts/start.py`가 시작 시 로더를 한 번 불러 원본 형식 오류를 조기에 드러내게 한다.
  - `/api/v4/brain/status`에 온톨로지 버전·클래스 수·등록부 브랜드 수·플래그 상태를 노출한다.
- **Pellet 검증은 배포 전에 한다.** 순서: 로컬 또는 CI에서 `check_ontology_owl.py` 통과 → 배포. Docker 빌드 단계에서 돌리는 방법(다단계 빌드: JDK 25 단계에서 검증만 하고, 런타임 이미지에는 Java를 넣지 않음)은 O6에서 선택 과제로 둔다.
- **운영에서 Pellet을 매일 돌리는 것**은 이번 계획(O0~O7)의 범위가 아니다. 크롤 후 새 SoS로 OWL 분류를 다시 계산하는 경우가 여기에 해당한다. 필요해지면 따로 결정한다.
  - 필요한 것: 런타임 이미지에 JRE 25 추가(`COPY --from=eclipse-temurin:25-jre`, 약 +100~200MB). 질의마다가 아니라 **일일 배치에서만** 호출한다. Java가 없으면 Python 규칙으로 되돌아간다.
  - 운영 판단은 Railway 메모리·빌드 시간 측정 뒤에 한다. Dockerfile·Railway 변경은 사용자 승인이 필요하다(§8).
...
- 원산지·그룹을 **추정으로 채우기**
- 평가 결과를 유리하게 다듬기
- 서비스 질의 경로에서 Java·외부 추론기 호출(OE2). 개발·검증 스크립트의 Pellet 사용은 허용(OE2-추가). Dockerfile에 Java 추가는 사용자 승인 없이 하지 않음(OE10)
...
시작 전에 OE1(온톨로지 원본 형식: JSON 스키마 권장 vs OWL 파일)만 나에게 한 번 확인받고, 나머지 설계 OE2~OE10은 다시 묻지 말고 따를 것.
