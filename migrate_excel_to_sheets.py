...
for _, row in df.iterrows():
    ...
    converted_row = [
        ...
        asin,                        # asin
        str(row['product_name_raw'])[:200] if pd.notna(row['product_name_raw']) else "",  # product_name
...
creds = Credentials.from_service_account_file(
    os.getenv('GOOGLE_SHEETS_CREDENTIALS_PATH', './config/google_credentials.json'),
    scopes=SCOPES
)
