...
class QueryAgent:
    ...
    def set_client(self, client: Any) -> None:
        ...
        if self._response_pipeline:
            self._response_pipeline.set_client(client)
    ...
    async def initialize(self) -> None:
        ...
        if self._client:
            self._response_pipeline.set_client(self._client)
    ...
    async def _gather_context(self, query: str, entities: Dict[str, List[str]]) -> Context:
        ...
        return Context(
            query=query,
            entities=entities,
            metrics_data=self._current_metrics,
            summary="컨텍스트 수집 불가"
        )

    async def _generate_response(self, query: str, context: Context) -> Response:
        """응답 생성"""
        if self._response_pipeline:
            decision = Decision(tool="direct_answer", reason="쿼리 응답", confidence=0.8)
