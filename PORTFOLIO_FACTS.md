...
> **동결 고지 (2026-09-17).** 이 문서는 기준 커밋 `2b04aa1`(2026-08-30) 시점의 기록이며 더 이상 갱신하지 않는다.
> 이후 사실은 `docs/portfolio/amore_architecture_evidence.md`가 대체한다. 본문은 당시 기록으로 보존한다.
> 현재와 다른 대표 항목: 골든셋 160문항 → 172문항, KG 1,028트리플 → 3,500트리플(2026-09-17 로컬),
> 가드레일 no-op → `9dc02aa`에서 구현, 소셜 수집기 "통합 예정" → `a2a1c64`에서 삭제.
> **오기재 1건:** 아래 피처 플래그 표의 `retriever.use_owl_strategy` "live" 판정은 틀렸다. 당시에도 호출처가
> `OWLRetrievalStrategy`에 없는 `docs_path` 인자를 넘겨 생성이 항상 실패했다(2026-02-15 `f049cb8`부터).
> 2026-09 사후 분석에서 발견해 연결을 수리했고, 플래그 기본값은 OFF로 바꿨다.
...
## 개선 사이클 7 — 2026-08-30 (IR 도메인 골든셋 신설, v8.1)

**골든셋이 없던 영역에는 결함도 보이지 않았을 뿐이다.** 코퍼스의 24%(IR 분기보고서)를
다루는 문항 12개를 넣자마자 시스템이 IR 질문의 75%를 검색 전에 거절하고 있었다는
사실이 드러났다. 상세: `docs/experiments/eval_cycle7_2026-08-30.md`. 전 과정 [실행 확인]

- **문항 12개 신설** (`scripts/add_ir_golden_items.py`, 멱등·dry-run·코퍼스 검증 내장):
  gold.answer의 모든 수치를 `docs/ir/AP_*Q25_EN.md` 원문에서 인용하고 각 문항에 근거
  청크와 원문 표현을 주석으로 남겼다. 한국어 8 / 영어 4로 섞어 사이클 6에서 고친
  교차언어 회수율을 골든셋 안에서 상시 감시한다.
- **라우터가 IR 질문 12개 중 9개를 UNKNOWN 폴백으로 거절**하고 있었다 —
  `RAGRouter.QUERY_PATTERNS`에 실적 어휘가 하나도 없어 검색·응답 파이프라인 진입
  자체가 막혀 있었다. 수정 후 IR 근거성 **0.192 → 0.757**, 적합성 0.183 → 0.592,
  L3 hits 0.667 → 1.000.
- 함께 드러난 결함 2건: 개념 매칭 substring 오탐(`ope|rating| profit` → review_rating,
  사이클 4 단어경계 규칙의 사각지대), 모기업 엔티티 미등록(KG엔 있는데 사전엔 없음
  → IR 문항 L1 entity F1 0.639 → **0.972**).

### 측정 재현성 한계 (이번 사이클에서 발견, 소급 적용)

코퍼스·코드가 사실상 같은 두 실행에서 **기존 160문항 중 98문항의 검색 결과가 달랐다.**
원인은 질의 확장 LLM의 `temperature=0.3`이었고 0으로 고정했다.
**사이클 4~7에서 보고한 0.03 이하 델타는 이 편차와 겹칠 수 있으므로 개선/회귀로
단정하지 않는다.** 포트폴리오에 소폭 델타를 성과로 기재하지 말 것.

### v7.1 → v8.1 (분모가 160 → 172로 바뀜, 부분집합으로 비교)

| 지표 | v7.1 (160) | v8.1의 같은 160 | v8.1 IR(12) | v8.1 전체(172) |
|---|---|---|---|---|
| Overall | 0.562 | 0.561 | **0.413** | 0.551 |
| 게이트 통과 | 17 | 16 | **0** | 16 |
| L1 Entity F1 | 0.606 | 0.611 | **0.972** | 0.636 |
| L2 개념 recall | 0.603 | 0.604 | **0.250** | 0.579 |
| L5 Groundedness | 0.678 | 0.686 | **0.757** | 0.691 |

기존 160문항은 사실상 변화 없음(0.562 → 0.561) — 이번 변경이 기존 문항을 해치지
않았다는 뜻이다. **IR은 라우터 수정으로 크게 올랐지만 여전히 최약 도메인이고
통과 문항은 0개다.** 원인도 특정했다: 인텐트 구성의 `top_k=5`와 평가의 recall@8이
어긋나 IR 12문항 중 6개가 IR 청크를 한 건도 회수하지 못한다. 추정으로 고치지 않고
다음 사이클에서 단독 검증한다.
